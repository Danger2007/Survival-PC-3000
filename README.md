...
4) **The analogical input is not completely loyal to the games**, the wheel is too small, and replacing it with a second rotary encoder occupies too much space,  there is no knob 3D print yet, as i don't have the measurements needed to make it and also I couldn't find a model of button similar to the ones in the games, so I decided to take the second best option, with that strange ring of yellow light.
